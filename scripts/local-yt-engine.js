        ...
        const finalData = await page.evaluate((url) => {
            ...
            items.forEach(item => {
                ...
                replyItems.forEach(r => {
                    replies.push({
                        text: r.querySelector('#content-text')?.innerText,
                        author: r.querySelector('#author-text')?.innerText.trim()
                    });
                });

                if (text) results.push({ video_url: url, text, author, likes, replies });
            });
            return results;
        }, videoUrl);

        fs.writeFileSync('comments_clean.json', JSON.stringify(finalData, null, 2));
        console.log(`✅ THÀNH CÔNG! Đã lưu ${finalData.length} thảo luận vào comments_clean.json`);
